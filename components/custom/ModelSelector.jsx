...
import { ChevronDown, Check, Zap, AlertCircle, Cpu } from 'lucide-react';
import { AI_MODELS, isOpenRouterConfigured } from '@/configs/AiModel';

function ModelSelector({ selectedModel, onModelChange, className = "", minimalist = false }) {
    ...
    if (minimalist) {
        return (
            <div className={`relative ${className}`}>
                {/* Minimalist Model Selector Button */}
                <button
                    onClick={() => setIsOpen(!isOpen)}
                    className="group relative flex items-center justify-center bg-slate-800/30 hover:bg-slate-700/40 border border-slate-600/30 hover:border-turquoise-500/40 rounded-lg px-3 py-2 transition-all duration-300"
                >
                    <div className="flex items-center space-x-2">
                        {/* Simple AI Icon */}
                        <Cpu className="h-4 w-4 text-turquoise-400" />
                        
                        {/* Model Name - Shortened */}
                        <span className="text-slate-300 text-sm font-medium">
                            {selectedModelConfig?.name?.split(' ')[0] || 'AI'}
                        </span>
                ...
                {/* Dropdown Menu */}
                {isOpen && (
                    <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-slate-800/95 backdrop-blur-xl border border-turquoise-500/20 rounded-xl shadow-2xl z-50 overflow-hidden min-w-[280px]">
                        {/* Header */}
                        <div className="p-3 border-b border-slate-700/50">
                            <div className="flex items-center space-x-2">
                                <Zap className="h-4 w-4 text-turquoise-400" />
                                <span className="text-sm font-medium text-turquoise-300">Select AI Model</span>
                            </div>
                        </div>
    ...
    // Original full selector for non-minimalist use
    return (
        <div className={`relative ${className}`}>
