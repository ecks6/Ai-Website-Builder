...
function ChatView() {
    ...
    const GetWorkSpaceData = async () => {
        ...
        setSelectedEnv(result?.selectedEnv || 'react'); // Get environment from workspace
    }
    ...
    const GetAiResponse = async () => {
        setLoading(true);
        const PROMPT = JSON.stringify(messages) + Prompt.getPrompt('CHAT_PROMPT', selectedEnv);
            ...
            <div className="bg-gradient-to-r from-turquoise-500/10 to-cyan-500/10 p-4 border-b border-turquoise-500/20">
                <div className="flex items-center space-x-3">
                    ...
                    <div className="text-xs text-slate-400 bg-slate-800/50 px-3 py-1 rounded-full">
                        {selectedEnv === 'react' ? 'React Mode' : 'HTML Mode'}
                    </div>
                    <div className="text-xs text-slate-400 bg-slate-800/50 px-3 py-1 rounded-full">
                ...
                <div className="relative">
                    <div className="flex gap-4">
                        <div className="flex-1 relative group">
                            <textarea
                                placeholder={`Ask me anything about your ${selectedEnv === 'react' ? 'React' : 'HTML'} project...`}
                                value={userInput}
                    ...
                    <div className="flex items-center justify-between mt-4">
                        ...
                        <div className="text-xs text-slate-500">
                            Powered by AI • {selectedEnv === 'react' ? 'React Mode' : 'HTML Mode'}
                        </div>
                    </div>
                </div>
            </div>
